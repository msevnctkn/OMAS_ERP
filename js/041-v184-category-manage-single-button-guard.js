(function(){
  ...
  function watch(){
    dedupe();
    const target=document.getElementById('invoiceAnalysisResults')||document.body;
    if(!target || target.__invoiceCategoryDedupeWatch) return;
    target.__invoiceCategoryDedupeWatch=true;
    new MutationObserver(schedule).observe(target,{childList:true,subtree:true});
  }
  const oldReport=window.renderInvoiceCategoryAnalysis;
  if(typeof oldReport==='function'){
    window.renderInvoiceCategoryAnalysis=function(){const r=oldReport.apply(this,arguments);dedupe();setTimeout(dedupe,50);return r;};
  }
  const oldEnsure=window.ensureCategoryPanel;
  if(typeof oldEnsure==='function'){
    window.ensureCategoryPanel=function(){const r=oldEnsure.apply(this,arguments);dedupe();setTimeout(dedupe,50);return r;};
  }
