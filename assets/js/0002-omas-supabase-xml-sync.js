(function () {
  ...
  async function tableCount(client, table, companyId) {
    var result = await client.from(table).select('id', { count: 'exact', head: true }).eq('company_id', companyId);
  ...
  async function syncXmlRowsToSupabase() {
    ...
    var realInvoices = await tableCount(auth.client, 'faturalar', auth.companyId);
    var realLines = await tableCount(auth.client, 'fatura_kalemleri', auth.companyId);
    stats.supabaseFaturalar = realInvoices;
    stats.supabaseKalemler = realLines;
    status('Supabase aktarımı tamamlandı. Veritabanı toplamı: Fatura: ' + realInvoices + ', benzersiz kalem: ' + realLines + '. İşlenen local kalem: ' + stats.kalemler + '.', true);
